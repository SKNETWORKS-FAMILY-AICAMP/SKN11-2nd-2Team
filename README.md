...
| 개선 포인트 | 설명 |
|-------------|------|
| 🎯 `predict_proba` 처리 검토 | 클래스 순서 확인, [1] 확률 사용 여부 |
| 🔁 클래스 레이블 반전 여부 점검 | `Target_Churn` 이 0/1로 일관성 있게 인코딩 되었는지 확인 |
| ⚖️ 모델 하이퍼파라미터 재조정 | `is_unbalance`, `scale_pos_weight`, `num_leaves` 등 튜닝 필요 |
| 🔍 다른 모델과 비교 | XGBoost, CatBoost 등과 성능 비교 추천 |
| 📈 Threshold 조정 실험 | ROC를 기준으로 최적 threshold 찾기 |

---

## ✅ 결론

> 현재 모델은 **정확도와 f1-score에서는 일정 수준의 성능을 보이지만**,  
> **확률 예측 관점에서는 신뢰도가 낮아 실전 적용 시 주의가 필요**합니다.  
> 특히 이탈 확률 기반 자동화 시스템 구축 시에는 AUC 개선이 선행되어야 하며,  
> **전처리 및 타겟 인코딩, 모델 튜닝 전반의 재검토**가 필요합니다.
